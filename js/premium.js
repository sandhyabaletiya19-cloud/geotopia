...
(function () {
    ...
    function init() {
        ...
        if (currentPlan !== 'basic') {
            var existingExpiry = localStorage.getItem('dv_plan_expiry');
            var expiry = existingExpiry || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString();

            var existing = localStorage.getItem('dharaverse_premium');
            var needsSync = true;
